        ...
        on_notification: function(notification) {
            ...
            if((Array.isArray(channel) && channel[0] &&(channel[0][1] === 'popup.reminder'))){
                if(channel[1]){
                    ...
                    self.$el.find(".oe_popup_notification").text(channel[1])
                }
            }
        },
        get_data: function(){
            var self = this
            $('#oe_main_menu_navbar').find('.read_button').on('click', function(e) {
                var message_id = $(this).attr('value');
                var notification_msg_model = new Model('notification.msg');
                notification_msg_model.call('update_notification_msg',[parseInt(message_id)]).done(function(){
                    $('button[value='+message_id+']').hide();
                    $('button[value='+message_id+']').closest("tr").css({"background-color": "gray"});
                });
            });
