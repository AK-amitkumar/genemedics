...
try:
    ...
    logging.getLogger(_("Power Email")).info(_("Mako templates not installed"))
    #LOGGER.notifyChannel(_("Power Email"),netsvc.LOG_ERROR,_("Mako templates not installed"))
try:
    ...
    logging.getLogger(_("Power Email")).info(_("Django templates not installed"))
    #LOGGER.notifyChannel(_("Power Email"),netsvc.LOG_ERROR,_("Django templates not installed"))
...
def get_value(cursor, user, recid, message=None, template=None, context=None):
    ...
    """
    pool = openerp.pooler.get_pool(cursor.dbname)
    if message is None:
        message = {}
    #Returns the computed expression
    if message:
        try:
            message = openerp.tools.ustr(message)
...
class poweremail_templates(osv.osv):
    ...
    def _generate_attach_reports(self,
        ...
        new_att_vals = {
            ...
            'datas_fname': openerp.tools.ustr(
                             get_value(
                                   cursor,
                                   user,
                                   record_ids[0],
    ...
    def _generate_mailbox_item_from_template(self,
        ...
        if lang:
            ...
            template = self.browse(cursor, user, template.id, context=ctx)
        mailbox_values = {
            'pem_from': openerp.tools.ustr(from_account['name']) + \
                        "<" + openerp.tools.ustr(from_account['email_id']) + ">",
