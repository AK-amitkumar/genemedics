...
#from openerp.template import get_value
...
class poweremail_send_wizard(osv.osv_memory):
    ...
    def save_to_mailbox(self, cr, uid, ids, context=None):
        ...
        for id in context['src_rec_ids']:
            ...
            vals = {
                'pem_from': openerp.tools.ustr(accounts['name']) + "<" + openerp.tools.ustr(accounts['email_id']) + ">",
            ...
            if screen_vals['signature']:
                ...
                if signature:
                    vals['pem_body_text'] = openerp.tools.ustr(vals['pem_body_text'] or '') + '\n--\n' + signature
                    vals['pem_body_html'] = openerp.tools.ustr(vals['pem_body_html'] or '') + signature
            ...
            if template.report_template:
                ...
                attachment_id = self.pool.get('ir.attachment').create(cr, uid, {
                    'name': _('%s (Email Attachment)') % openerp.tools.ustr(vals['pem_subject']),
                    'datas': base64.b64encode(result),
                    'datas_fname': openerp.tools.ustr(get_end_value(id, screen_vals['report']) or _('Report')) + "." + format,
