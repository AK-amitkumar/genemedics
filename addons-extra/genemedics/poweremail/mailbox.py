...
class PoweremailMailbox(osv.osv):
    ...
    def send_this_mail(self, cr, uid, ids=None, context=None):
        ...
        print "send mails one by one",ids
        for id in ids:
            print "111111111"
            try:
                print "22222222222"
                ...
                #logger = netsvc.Logger()
                logging.getLogger(_("Power Email")).info(_("Sending of Mail %s failed. Probable Reason: Could not login to server\nError: %s") % (id, error))
    ...
    def historise(self, cr, uid, ids, message='', context=None):
        for id in ids:
            ...
            self.write(cr, uid, id, {'history':history or '' + "\n" + time.strftime("%Y-%m-%d %H:%M:%S") + ": " + openerp.tools.ustr(message)}, context)
