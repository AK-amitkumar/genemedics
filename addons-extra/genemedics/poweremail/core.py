...
class poweremail_core_accounts(osv.osv):
    ...
    def _get_outgoing_server(self, cursor, user, account_id, context=None):
        ...
        if account.smtpuname and account.smtppass:
            server.login(
    ...
    def smtp_connection(self, cursor, user, id, context=None):
        ...
        if core_obj.smtpserver and core_obj.smtpport and core_obj.state == 'approved':
            try:
                ...
                logging.getLogger(_("Power Email")).info(_("Mail from Account %s failed on login. Probable Reason: Could not login to server\nError Description: %s") % (id, error))  
        ...
        for id in ids:
            ...
            if serv:
                try:
                    ...
                    msg['Organization'] = openerp.tools.ustr(core_obj.user.company_id.name)
                    msg['Date'] = formatdate()
...
#                        msg['BCC'] = u','.join(addresses_l['BCC'])
                    ...
                    if core_obj.send_pref == 'text' or core_obj.send_pref == 'both':
                        ...
                        body_text = openerp.tools.ustr(body_text)
                    ...
                    if core_obj.send_pref == 'html' or core_obj.send_pref == 'both':
                        ...
                        html_body = openerp.tools.ustr(html_body)
                    ...
                    logging.getLogger(_("Power Email")).info(_("Mail from Account %s failed. Probable Reason: MIME Error\nDescription: %s") % (id, error))  
                ...
                try:
                    ...
                    logging.getLogger(_("Power Email")).info(_("Mail from Account %s failed. Probable Reason: Server Send Error\nDescription: %s") % (id, error))  
    ...
    def save_header(self, cr, uid, mail, coreaccountid, serv_ref, context=None):
        ...
        #print vals
            ...
            logging.getLogger(_("Power Email")).info(_("Save Header -> Mailbox create error. Account: %s, Mail: %s, Error: %s") % (coreaccountid,serv_ref, str(e)))
        ...
        if crid:
            logging.getLogger(_("Power Email")).info(_("Header for Mail %s Saved successfully as ID: %s for Account: %s.") % (serv_ref, crid, coreaccountid))
            ...
            logging.getLogger(_("Power Email")).info(_("IMAP Mail -> Mailbox create error. Account: %s, Mail: %s") % (coreaccountid, serv_ref))
    ...
    def complete_mail(self, cr, uid, mail, coreaccountid, serv_ref, mailboxref, context=None):
        ...
        parsed_mail = self.get_payloads(mail)
        vals['pem_body_text'] = openerp.tools.ustr(parsed_mail['text'])
        vals['pem_body_html'] = openerp.tools.ustr(parsed_mail['html'])
        #Create the mailbox item now
        crid = False
        try:
    ...
    def get_mails(self, cr, uid, ids, context=None):
        ...
        for id in ids:
            logging.getLogger(_("Power Email")).info(_("Starting Header reception for account: %s.") % (id))
            ...
            if rec:
                if rec.iserver and rec.isport and rec.isuser and rec.ispass :
                    if rec.iserver_type == 'imap' and rec.isfolder:
                        ...
                        logging.getLogger(_("Power Email")).info(_("IMAP Folder Statistics for Account: %s: %s") % (id, serv.status('"%s"' % rec.isfolder, '(MESSAGES RECENT UIDNEXT UIDVALIDITY UNSEEN)')[1][0]))
                        ...
                        logging.getLogger(_("Power Email")).info(_("POP3 Statistics: %s mails of %s size for Account: %s") % (serv.stat()[0], serv.stat()[1], id))
                        ...
                        logging.getLogger(_("Power Email")).info(_("Incoming server login attempt dropped Account: %s Check if Incoming server attributes are complete.") % (id))
        ...
        if rec:
            if rec.iserver and rec.isport and rec.isuser and rec.ispass :
                if rec.iserver_type == 'imap' and rec.isfolder:
                    ...
                    logging.getLogger(_("Power Email")).info(_("Incoming server login attempt dropped Account: %s\nCheck if Incoming server attributes are complete.") % (id))
    ...
    def get_payloads(self, mail):
        ...
        for part in mail.walk():
            ...
            if mail_part_type == 'text/plain':
                parsed_mail['text'] = openerp.tools.ustr(part.get_payload(decode=True)) # decode=True to decode a MIME message
            elif mail_part_type == 'text/html':
                parsed_mail['html'] = openerp.tools.ustr(part.get_payload(decode=True)) # Is decode=True needed in html MIME messages?
